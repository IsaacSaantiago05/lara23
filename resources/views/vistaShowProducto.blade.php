	...
	<div class="container">
		<div class="card">
			...
			<div class="grid">
				...
				<strong>Descripcion</strong><span>{{ $product->description }}</span>
				<strong>Descripcion larga</strong><span>{{ $product->descriptionLong }}</span>
				<strong>Precio</strong><span>{{ $product->price }}</span>
				<strong>Categoria</strong><span>{{ $product->category?->name ?? 'Sin categoria' }}</span>
			</div>

			<div class="actions">
				<a class="btn" href="{{ route('Productos.edit', $product->id) }}">Editar</a>
				<a class="btn btn-secondary" href="{{ route('Productos') }}">Volver al listado</a>
			</div>
		</div>
	</div>
</body>
